...
// 让所有 Canvas 2D fillText / strokeText 都经过 cnItem() 翻译
// DOM 和 Canvas 共用同一个 cnItem，无需两套字典
!function() {
        ...
        if (contextType === '2d' && ctx && !ctx.__cnHooked) {
            ...
            ctx.fillText = function(text, x, y, maxWidth) {
                if (maxWidth !== undefined) {
                    return origFillText.call(this, cnItem(text), x, y, maxWidth);
                }
                return origFillText.call(this, cnItem(text), x, y);
            };
            ctx.strokeText = function(text, x, y, maxWidth) {
                if (maxWidth !== undefined) {
                    return origStrokeText.call(this, cnItem(text), x, y, maxWidth);
                }
    ...
    console.log("Canvas 2D 汉化钩子已加载 → 统一 cnItem");
}();